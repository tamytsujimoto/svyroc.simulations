...
roc_var_svy_ipw <- function(roc_data,
                            design,
                            prop_score,
                            grid = NULL){
  ...
  roc_var <-
    ...
    dplyr::mutate(ratio = zoo::na.approx(ratio, na.rm = FALSE),
                  v1 = p1^(-1)*tpr*(1-tpr) + (1-p1)^(-1)*ratio^2*fpr*(1-fpr),
                  vsvy_ipw_fin = (1/n)*(sampling_frac*mu_pi1/p_phi)*v1,
                  vsvy_ipw_sup = (1/n)*(sampling_frac*(1+mu_pi1)/p_phi)*v1) %>%
    dplyr::select(-c(fpr, aux, f1, f0, ratio, v1)) %>%
